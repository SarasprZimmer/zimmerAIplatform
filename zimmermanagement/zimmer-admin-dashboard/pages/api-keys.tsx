...
const APIKeysPage: React.FC = () => {
  ...
  const fetchKeys = async () => {
    try {
      let url = `${process.env.NEXT_PUBLIC_API_URL || "http://193.162.129.243:8000"}/api/admin/list`;
  ...
  const fetchAutomations = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://193.162.129.243:8000"}/api/admin/automations`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (response.ok) {
        const data = await response.json();
  ...
  const handleCreate = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://193.162.129.243:8000"}/api/admin/`, {
        method: 'POST',
        headers: {
  ...
  const handleUpdate = async () => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://193.162.129.243:8000"}/api/admin/keys/${selectedKey.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
  ...
  const handleStatusUpdate = async (keyId: number, status: string) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://193.162.129.243:8000"}/api/admin/keys/${keyId}/status`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
  ...
  const handleTest = async (keyId: number) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://193.162.129.243:8000"}/api/admin/keys/${keyId}/test`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` }
      });
