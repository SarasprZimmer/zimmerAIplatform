...
export default function Backups() {
  ...
  const fetchBackups = async () => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "https://api.zimmerai.com"}/api/admin/backups?${params}`, {
        headers: {
  ...
  const fetchStats = async () => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "https://api.zimmerai.com"}/api/admin/backups/stats`, {
        headers: {
          'Authorization': `Bearer ${authClient.getAccessToken()}`,
          'Content-Type': 'application/json'
        }
  ...
  const triggerBackup = async () => {
    ...
    setError(``);
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "https://api.zimmerai.com"}/api/admin/backups/trigger`, {
        method: `POST`,
        headers: {
          'Authorization': `Bearer ${authClient.getAccessToken()}`,
          'Content-Type': 'application/json'
  ...
  const verifyBackup = async (backupId: number) => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "https://api.zimmerai.com"}/api/admin/backups/verify/${backupId}`, {
        method: `POST`,
        headers: {
          'Authorization': `Bearer ${authClient.getAccessToken()}`,
          'Content-Type': 'application/json'
  ...
  const cleanupBackups = async () => {
    ...
    setError(``);
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "https://api.zimmerai.com"}/api/admin/backups/cleanup?retention_days=7`, {
        method: `DELETE`,
        headers: {
          'Authorization': `Bearer ${authClient.getAccessToken()}`,
          'Content-Type': 'application/json'
