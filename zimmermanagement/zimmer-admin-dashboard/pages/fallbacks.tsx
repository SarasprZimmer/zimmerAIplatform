...
export default function Fallbacks() {
  ...
  const fetchClients = async () => {
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://api.zimmerai.com'}/api/admin/users`, {
        headers: {
  ...
  const fetchFallbacks = async () => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://api.zimmerai.com'}${url}`, {
        headers: {
          'Authorization': `Bearer ${authClient.getAccessToken()}`,
          'Content-Type': 'application/json'
        }
