...
if os.path.exists(file_path):
    ...
    # Find the current delete_automation function and replace it completely
    # This regex will match from @router.delete to the next @router or end of file
    pattern = r'@router\.delete\("/\{automation_id\}"[^)]*\)\s*async def delete_automation\([^)]*\):.*?(?=\n@router\.|\Z)'
...
async def delete_automation(
    ...
    """Delete an automation using separate database connections to avoid transaction issues"""
    ...
    # Create a new database engine for separate connections
