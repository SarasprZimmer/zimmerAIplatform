...
import { DESIRED_FEATURES } from "./feature_inventory.config";
...
function guessRouteFromFile(f: string) {
...
async function main() {
  ...
  for (const f of files) {
    ...
    const endpoints: PageInfo["endpoints"] = [];
    const hrefs: string[] = [];
  ...
  const featureByRoute: Record<string, { endpoints: string[]; methods: string[]; charts: string[]; hrefs: string[]; components: string[] }> = {};
...
function matchEndpointPattern(ep: string, pat: string) {
...
function renderMarkdownReport(
  pages: any[],
  byRoute: Record<string, { endpoints: string[]; methods: string[]; charts: string[]; hrefs: string[]; components: string[] }>,
  coverage: any[],
  missing: any[],
) {
  const lines: string[] = [];
  ...
  for (const p of pages.sort((a,b)=>a.route.localeCompare(b.route))) {
    ...
    if (p.endpoints.length) lines.push(`- **Endpoints**: ${p.endpoints.map((e:any)=>"`"+(e.path||e)+"`").join(", ")}`);
  ...
  for (const c of coverage) {
    ...
    if (c.usedByRoutes.length) {
      ...
      lines.push(`  - **Endpoints**: ${c.matchedEndpoints.map((e:string)=>"`"+e+"`").join(", ")}`);
    }
    lines.push("");
  }
  ...
  if (missing.length) {
    ...
    for (const m of missing) {
      ...
      if (m.expectedEndpoints) {
        lines.push(`  - Expected endpoints: ${m.expectedEndpoints.map((e: string) => "`" + e + "`").join(", ")}`);
      }
    }
    lines.push("");
  }
  ...
  lines.push("");
  const epToRoutes: Record<string, Set<string>> = {};
